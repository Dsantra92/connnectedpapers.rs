...
impl ConnectedPapersClient {
    ...
    ) -> Pin<Box<dyn Stream<Item = Result<GraphResponse, Box<dyn std::error::Error + Send + Sync>>>>>
    {
        ...
        Box::pin(stream::unfold(
            ...
            move |(mut retry_counter, mut newest_graph, mut last_error, mut fresh_only)| {
                ...
                async move {
                    while retry_counter > 0 {
                        let response = client
                            .get(format!(
                                "{}/papers-api/graph/{}/{}",
                                server_addr_cloned, fresh_only as i32, paper_id_cloned
                            ))
                            ...
                            Ok(resp) if resp.status() == 200 => {
                                match resp.json::<Value>().await {
                                    Ok(mut body) => {
                                        ...
                                        let status = body.get("status").and_then(|v| {
                                            serde_json::from_value::<GraphResponseStatuses>(
                                                v.clone(),
                                            )
                                            .ok()
                                        });
                                        ...
                                        match serde_json::from_value::<GraphResponse>(body) {
                                            Ok(mut data) => {
                                                ...
                                                if is_end_status(&data.status) || !loop_until_fresh
                                                {
                                                    ...
                                                    return Some((
                                                        Ok(data),
                                                        (0, newest_graph, None, fresh_only),
                                                    ));
                                                }
                                                data.graph_json = newest_graph.clone();
                                                return Some((
                                                    Ok(data),
                                                    (retry_counter, newest_graph, None, fresh_only),
                                                ));
                                            }
                        ...
                        if last_error.is_some() {
                            ...
                            if retry_counter > 0 {
                                sleep(Duration::from_millis(SLEEP_TIME_AFTER_ERROR)).await;
                                // Sleep after an error before retrying
                            }
                        } else {
